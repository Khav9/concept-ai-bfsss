...
function spiralWallOrder() {
  const walls = [];
  let top = 0, left = 0, bottom = ROWS - 1, right = COLS - 1, dir = 0;
  while (top <= bottom && left <= right) {
    if      (dir === 0) { for (let c = left;   c <= right;  c++) walls.push([top, c]);    top    += 2; }
    else if (dir === 1) { for (let r = top;    r <= bottom; r++) walls.push([r, right]);  right  -= 2; }
    else if (dir === 2) { for (let c = right;  c >= left;   c--) walls.push([bottom, c]); bottom -= 2; }
    else                { for (let r = bottom; r >= top;    r--) walls.push([r, left]);   left   += 2; }
    dir = (dir + 1) % 4;
  }
...
function randomWallOrder(sR, sC, tR, tC) {
  const walls = [];
  for (let r = 0; r < ROWS; r++)
    for (let c = 0; c < COLS; c++)
      if (!(r === sR && c === sC) && !(r === tR && c === tC) && Math.random() < 0.28)
        walls.push([r, c]);
...
const nbrs = (r, c) => [[r-1,c],[r+1,c],[r,c-1],[r,c+1]];
const key = (r, c) => `${r},${c}`;
...
export default function App() {
  ...
  const [phase,      setPhase    ] = useState('idle'); // idle|maze|search|done
  const [algo,       setAlgo     ] = useState('BFS');
  const [mazeType,   setMazeType ] = useState('spiral');
  ...
  useEffect(() => {
    const h = () => { setAlgoOpen(false); setMazeOpen(false); setSpeedOpen(false); };
  ...
  function toggleCell(r, c) {
    if (running) return;
    if ((r === START.r && c === START.c) || (r === TGT.r && c === TGT.c)) return;
    setCellStates(prev => {
  ...
  // ── BFS / DFS search ──────────────────────────────────────────────────
  function runSearch(wallGrid, baseStates, ms) {
      ...
      if (i < visitOrder.length) {
        ...
        function stepPath() {
          ...
          for (let b = 0; b < batchP && pi < path.length; b++, pi++) {
            const idx = path[pi];
            if (working[idx] !== 4 && working[idx] !== 5) working[idx] = 3;
          }
          setCellStates(new Uint8Array(working));
  ...
  const algoOptions = [
    { id: 'BFS', label: 'BFS — Shortest path guaranteed', desc: 'Explores layer by layer' },
    { id: 'DFS', label: 'DFS — Explores deep first',      desc: 'Not necessarily shortest' },
  ];
  const mazeOptions = [
  ...
  const styles = {
    ...
    logo: { fontWeight: 800, fontSize: 16, marginRight: 12, letterSpacing: '-0.02em' },
    ...
    swatchLabel: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, color: '#475569' },
    main: { display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '24px 16px', background: '#f1f5f9', minHeight: 'calc(100vh - 100px)' },
    grid: { border: '1px solid #cbd5e1', background: '#f8fafc', display: 'inline-block', lineHeight: 0, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' },
    toast: {
      position: 'fixed', bottom: 20, right: 20,
      background: '#0f0a1e', color: '#fff',
      borderRadius: 10, padding: '12px 16px',
      fontSize: 13, maxWidth: 220,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: '#f1f5f9' }}>
      ...
      <nav style={styles.nav}>
        ...
        <div style={styles.dd} onMouseDown={e => e.stopPropagation()}>
          <button
            ...
            onClick={() => { setAlgoOpen(v => !v); setMazeOpen(false); setSpeedOpen(false); }}
          >
            {algo} <span style={{ fontSize: 10, opacity: 0.6 }}>▾</span>
          </button>
          {algoOpen && (
            <div style={styles.ddMenu}>
              {algoOptions.map(a => (
                <button key={a.id} style={styles.ddItem(algo === a.id)}
                  onClick={() => { setAlgo(a.id); setAlgoOpen(false); }}>
                  {a.label}
                </button>
              ))}
            </div>
        ...
        <div style={styles.dd} onMouseDown={e => e.stopPropagation()}>
          <button
            disabled={running}
            style={styles.ddBtn(running)}
            onClick={() => { setMazeOpen(v => !v); setAlgoOpen(false); setSpeedOpen(false); }}
          >
          ...
          {mazeOpen && (
            <div style={styles.ddMenu}>
              {mazeOptions.map(m => (
                <button key={m.id} style={styles.ddItem(mazeType === m.id)}
                  onClick={() => { setMazeType(m.id); setMazeOpen(false); }}>
                  {m.label}
                </button>
              ))}
            </div>
        ...
        <div style={styles.dd} onMouseDown={e => e.stopPropagation()}>
          <button
            disabled={running}
            style={styles.ddBtn(running)}
            onClick={() => { setSpeedOpen(v => !v); setAlgoOpen(false); setMazeOpen(false); }}
          >
            Speed: {speed} <span style={{ fontSize: 10, opacity: 0.6 }}>▾</span>
          </button>
          {speedOpen && (
            <div style={styles.ddMenu}>
              {['Slow','Medium','Fast'].map(s => (
                <button key={s} style={styles.ddItem(speed === s)}
        ...
        {running && (
          <div style={styles.badge}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 600 }}>
              {phase === 'maze' ? 'Building maze…' : `${algo} running…`}
            </span>
          </div>
        )}
      ...
      <div style={styles.legend}>
        ...
        {phase === 'done' && pathLen > 0 && (
          <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 700, color: '#c084fc' }}>
      ...
      {toast && (
        <div style={styles.toast}>
          <div style={{ color: '#c084fc', fontWeight: 700, fontSize: 15, marginBottom: 4 }}>
            {phase === 'done' && pathLen > 0 ? '✓' : phase === 'done' ? '✗' : '◉'}
          </div>
          <div>{toast}</div>
        </div>
      )}
